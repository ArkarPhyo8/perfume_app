...
export default function AdminNavbar() {
  //Admin data
  const { adminInfo } = useContext(CardProviderContext);
      ...
      <header className="bg-[#1E1B4B] py-5 px-5 sticky top-0 z-50 border-b-2 border-[#1D4ED8]">
        <div className="flex items-center justify-between max-w-[1440px] mx-auto">
          ...
          <div className="flex gap-5 items-center">
            ...
            {adminInfo && (
              <div className="text-white font-medium text-md px-3 py-1 border-2 border-[#ce1f6d] bg-[#DB2777] rounded-full cursor-pointer">
                <h1>{adminInfo.name.split(" ",1)}</h1>
              </div>
            )}
          </div>
        </div>
